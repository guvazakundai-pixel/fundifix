// FundiFix Premium Redesign — App Controller

let mapInstance = null;
let mapMarkers = [];
let activeUser = JSON.parse(localStorage.getItem('fundifix_user')) || null;
      ...
      <div>
        <div class="profile-card-cta">
          ...
          <div style="display:flex;flex-direction:column;gap:12px;">
            <a href="https://wa.me/${tech.whatsapp}?text=Hi%20${encodeURIComponent(tech.name)},%20I%20found%20you%20on%20FundiFix%20and%20need%20a%20phone%20repair." target="_blank" class="btn btn-accent" style="height:48px;border-radius:var(--radius-md);font-size:15px;text-decoration:none;"><i data-lucide="message-square-text"></i> Message on WhatsApp</a>
          ...
          <div style="font-size:12px;color:var(--text-secondary);text-align:center;padding-top:8px;border-top:1px solid var(--color-border);"><i data-lucide="shield-check" style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:4px;"></i> FundiFix safe platform validation</div>
        </div>
      </div>
    </div>`;
...
function mockGoogleAuth() {
  ...
  localStorage.setItem('fundifix_user', JSON.stringify(activeUser));
  updateAuthBtnState();
...
function mockEmailAuth() {
  activeUser = { name: 'Email User', email: 'user@fundifix.co.za', avatar: '' };
  localStorage.setItem('fundifix_user', JSON.stringify(activeUser));
  updateAuthBtnState();
  closeAuthModal(null, true);
...
function updateAuthBtnState() {
  ...
  if (activeUser) {
    ...
    btn.onclick = () => { if (confirm('Sign out of FundiFix?')) { activeUser = null; localStorage.removeItem('fundifix_user'); updateAuthBtnState(); } };
  } else {
